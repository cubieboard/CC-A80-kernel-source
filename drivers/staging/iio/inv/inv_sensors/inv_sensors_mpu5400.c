...
#define MPU5400_SMPLRT_DIV		0x19
#define MPU5400_CONFIG 			0x1A
#define MPU5400_GYRO_CONFIG		0x1B
#define MPU5400_ACCEL_CONFIG	0x1C
#define MPU5400_ACCEL_CONFIG2	0x1D
#define MPU5400_LP_ACCEL_ODR   0x1E
#define MPU5400_WOM_THRESH	0x1F
#define MPU5400_FIFO_EN			0x23
#define MPU5400_INT_PIN_CFG		0x37
#define MPU5400_INT_ENABLE		0x38
#define MPU5400_DMP_INT_STATUS	0x39
#define MPU5400_INT_STATUS		0x3A
#define MPU5400_ACCEL_XOUT_H	0x3B
#define MPU5400_TEMP_OUT_H	0x41
#define MPU5400_GYRO_XOUT_H	0x43
#define MPU5400_ACCEL_INTEL_CTRL 0x69
#define MPU5400_USER_CTRL		0x6A
#define MPU5400_PWR_MGMT_1		0x6B
#define MPU5400_PWR_MGMT_2		0x6C
#define MPU5400_PRGM_STRT_ADDRH     0x70
#define MPU5400_FIFO_COUNTH		0x72
#define MPU5400_FIFO_R_W		0x74
#define MPU5400_WHOAMI			0x75
...
struct mpu5400_ctrl_t {
	u16 i2c_addr;
	struct invsens_i2c_t *i2c_handle;

	struct mpu5400_reg_cfg_t reg;
	struct mpu5400_reg_backup_t reg_backup;
	...
	bool is_mi_enabled;
	bool is_lpa_enabled;
	bool is_gyro_requested;	//requested by DMP
};



static const u16 mpu5400_st_tb[256] = {
...
static int mpu5400_set_configuration(struct mpu5400_ctrl_t *dctl)
{
	int res = 0;
...
static int mpu5400_check_silicon_rev(struct mpu5400_ctrl_t *dctl)
{
	int res = SM_SUCCESS;
...
static int mpu5400_restore_register(struct mpu5400_ctrl_t *dctl)
{
	int res = SM_SUCCESS;

	INV_DBG_FUNC_NAME;

	i2c_write_single(dctl, MPU5400_CONFIG,
			 dctl->reg_backup.config, res, exit_i2c_error);
	i2c_write_single(dctl, MPU5400_GYRO_CONFIG,
			 dctl->reg_backup.gyro_config, res,
			 exit_i2c_error);
	i2c_write_single(dctl, MPU5400_ACCEL_CONFIG,
			 dctl->reg_backup.accel_config, res,
			 exit_i2c_error);
	i2c_write_single(dctl, MPU5400_ACCEL_CONFIG2,
			 dctl->reg_backup.accel_config2, res,
			 exit_i2c_error);
	i2c_write_single(dctl, MPU5400_SMPLRT_DIV,
			 dctl->reg_backup.sample_div, res, exit_i2c_error);
	i2c_write_single(dctl, MPU5400_FIFO_EN, dctl->reg_backup.fifo_en,
			 res, exit_i2c_error);
	i2c_write_single(dctl, MPU5400_USER_CTRL,
			 dctl->reg_backup.user_ctrl, res, exit_i2c_error);
	i2c_write_single(dctl, MPU5400_INT_ENABLE,
			 dctl->reg_backup.int_enable, res, exit_i2c_error);

	i2c_write_single(dctl, MPU5400_PWR_MGMT_2,
...
static unsigned long mpu5400_sqrt(unsigned long x)
{
...
static int mpu5400_sw_selftest(struct mpu5400_ctrl_t *dctl,
		       ...
		       int *pkcount)
{
	...
	i2c_read(dctl, MPU5400_WHOAMI, 1, regs, res, exit_i2c_error);
	...
	/*-------------------------------------------------*/
	res = mpu5400_backup_register(dctl);
	if (res) {
		INVSENS_LOGE("register backup error = %d\n", res);
		goto exit_i2c_error;
	}

	i2c_read(dctl, MPU5400_PWR_MGMT_1, 2, regs, res, exit_i2c_error);


	tmp = (BIT_ACCEL_STBY);
	i2c_write_single(dctl, MPU5400_PWR_MGMT_2, tmp, res,exit_i2c_error);
	/*disable interrupt */
	i2c_write_single(dctl, MPU5400_INT_ENABLE, 0x0, res, exit_i2c_error);
	i2c_write_single(dctl, MPU5400_FIFO_EN, 0x0, res, exit_i2c_error);
	/*clear user_ctrl */
	i2c_write_single(dctl, MPU5400_USER_CTRL, 0x0, res, exit_i2c_error);
	/*reset fifo */
	i2c_write_single(dctl, MPU5400_USER_CTRL, BIT_FIFO_RST, res,
		exit_i2c_error);
	/*gyro set dlpf = 41hz */
	i2c_write_single(dctl, MPU5400_CONFIG, DLPF_CFG_41HZ, res, exit_i2c_error);
	/*accel set dlpf = 41hz */
	i2c_write_single(dctl, MPU5400_ACCEL_CONFIG2, A_DLPF_CFG_41HZ,
			 res, exit_i2c_error);
	/*set update rate = 1000Hz/(1+SMPLRT_DIV) */
	i2c_write_single(dctl, MPU5400_SMPLRT_DIV, 0x0, res, exit_i2c_error);
	/*set gyro fsr = 2000dps */
	i2c_write_single(dctl, MPU5400_GYRO_CONFIG, DEF_SW_SELFTEST_GYRO_FS,
			 res, exit_i2c_error);
	/*set accel fsr = 2g */
	i2c_write_single(dctl, MPU5400_ACCEL_CONFIG, AFSR_2G, res, exit_i2c_error);
	/*enable fifo */
	i2c_write_single(dctl, MPU5400_USER_CTRL, BIT_FIFO_EN, res,
			 exit_i2c_error);
	...
	/*enable fifo */
	tmp = BITS_GYRO_OUT;
	i2c_write_single(dctl, MPU5400_FIFO_EN, tmp, res, exit_i2c_error);
	...
	/*turn off fifo */
	i2c_write_single(dctl, MPU5400_FIFO_EN, 0x0, res, exit_i2c_error);
	i2c_read(dctl, MPU5400_FIFO_COUNTH, 2, regs, res, exit_i2c_error);
	...
	for (i = 0; i <DEF_SW_SKIP_COUNT; i++) {
		...
		i2c_read(dctl, MPU5400_FIFO_R_W, packet_size,
	...
	gyro_rms[0] = mpu5400_sqrt(gyro_rms[0] /packet_count);
	gyro_rms[1] = mpu5400_sqrt(gyro_rms[1] /packet_count);
	gyro_rms[2] = mpu5400_sqrt(gyro_rms[2] /packet_count);
	...
	*pkcount = packet_count;
	...
	/*clear int status */
	i2c_read(dctl, MPU5400_INT_STATUS, 1, regs, res, exit_i2c_error);

	mpu5400_restore_register(dctl);
...
static int mpu5400_do_test(struct mpu5400_ctrl_t *dctl, int self_test_flag,
			  int gyro_result[THREE_AXIS], int accel_result[THREE_AXIS])
{
	...
	i2c_write_single(dctl, MPU5400_INT_ENABLE, 0, res, exit_i2c_error);
	i2c_write_single(dctl, MPU5400_FIFO_EN, 0, res, exit_i2c_error);
	/* disable fifo reading */
	i2c_write_single(dctl, MPU5400_USER_CTRL, 0, res, exit_i2c_error);
	/* clear FIFO */
	i2c_write_single(dctl, MPU5400_USER_CTRL, BIT_FIFO_RST, res,
			 exit_i2c_error);
	/* setup parameters */
	i2c_write_single(dctl, MPU5400_CONFIG, DLPF_CFG_98HZ, res,
			 exit_i2c_error);
	...
	if (has_accel) {
		...
		d |= self_test_flag;
		i2c_write_single(dctl, MPU5400_ACCEL_CONFIG, d, res,
				 exit_i2c_error);
	}
	...
	while (s < INIT_ST_SAMPLES) {
		i2c_write_single(dctl, MPU5400_FIFO_EN, d, res,
				 exit_i2c_error);
		mdelay(DEF_GYRO_WAIT_TIME);
		i2c_write_single(dctl, MPU5400_FIFO_EN, 0, res,
				 exit_i2c_error);

		i2c_read(dctl, MPU5400_FIFO_COUNTH,
		...
		while (i < packet_count) {
			short vals[3];
			i2c_read(dctl, MPU5400_FIFO_R_W,
...
static int mpu5400_gyro_self_test(struct mpu5400_ctrl_t *dctl,
	...
	for (i = 0; i < 3; i++) {
		if (regs[i] != 0) {
			st_shift_prod[i] = mpu5400_st_tb[regs[i] - 1];
		} else {
			st_shift_prod[i] = 0;
			otp_value_zero = 1;
		}
...
static int mpu5400_accel_self_test(struct mpu5400_ctrl_t *dctl,
...
static int mpu5400_hw_selftest(struct mpu5400_ctrl_t *dctl,
	int *accel_st_ratio, int *gyro_st_ratio)
{
	...
	i2c_read(dctl, MPU5400_PWR_MGMT_2, 1, regs, res, exit_i2c_error);

	regs[0] &= ~((BIT_ACCEL_STBY) | (BIT_GYRO_STBY));

	i2c_write_single(dctl, MPU5400_PWR_MGMT_2, regs[0], res, exit_i2c_error);
	...
	while (test_times > 0) {
		res = mpu5400_do_test(dctl, 0, gyro_bias_regular,
			accel_bias_regular);
		if (res == -EAGAIN)
			test_times--;
		else
	...
	while (test_times > 0) {
		res = mpu5400_do_test(dctl, BITS_SELF_TEST_EN, gyro_bias_st,
					accel_bias_st);
		if (res == -EAGAIN)
			test_times--;
		else
	...
	accel_result = !mpu5400_accel_self_test(dctl,
		accel_bias_regular, accel_bias_st, accel_st_ratio);
	gyro_result = !mpu5400_gyro_self_test(dctl,
...
static int mpu5400_setup_hw(struct mpu5400_ctrl_t *dctl)
{
	int res = 0;
	u8 regs[2];
	...
	mpu5400_set_configuration(dctl);

	/*reset hw */
	i2c_write_single(dctl, MPU5400_PWR_MGMT_1, BIT_H_RESET, res,
			 exit_i2c_error);

	mdelay(30);

	/*wake hw */
	i2c_write_single(dctl, MPU5400_PWR_MGMT_1, 0x0, res, exit_i2c_error);

	/*check silicon revision */
	res = mpu5400_check_silicon_rev(dctl);

	/*set gyro fsr */
	regs[0] = dctl->reg.gyro_fsr;
	i2c_write_single(dctl, MPU5400_GYRO_CONFIG, regs[0], res, exit_i2c_error);

	/*set gyro dlpf */
	regs[0] = dctl->reg.gyro_dlpf | EXT_SYNC_SET_TEMP;
	i2c_write_single(dctl, MPU5400_CONFIG, regs[0], res, exit_i2c_error);

	/*set accel fsr */
	regs[0] = dctl->reg.accel_fsr;
	i2c_write_single(dctl, MPU5400_ACCEL_CONFIG, regs[0], res,
			 exit_i2c_error);

	/*set accel dlpf */
	regs[0] = dctl->reg.accel_dlpf;
	i2c_write_single(dctl, MPU5400_ACCEL_CONFIG2, regs[0], res,
			 exit_i2c_error);

	/*int cfg : set bypass as defaut */
	regs[0] = dctl->reg.int_pin_cfg;
	i2c_write_single(dctl, MPU5400_INT_PIN_CFG, regs[0], res, exit_i2c_error);

	regs[0] = dctl->reg.prog_start_addr >> 8;
	regs[1] = dctl->reg.prog_start_addr & 0xFF;
	i2c_write_single(dctl, MPU5400_PRGM_STRT_ADDRH, regs[0], res,
			 exit_i2c_error);
	i2c_write_single(dctl, MPU5400_PRGM_STRT_ADDRH + 1, regs[1], res,
			 exit_i2c_error);
...
static int mpu5400_set_delay(struct mpu5400_ctrl_t *dctl, u32 func_mask,
			     long delay, bool reset)
{
	...
	if (dctl->enabled_mask) {
		...
		i2c_write_single(dctl, MPU5400_SMPLRT_DIV, reg, res, exit_i2c_error);
		...
		i2c_write_single(dctl, MPU5400_INT_ENABLE, reg, res, exit_i2c_error);
...
static int mpu5400_set_motion_int(struct mpu5400_ctrl_t *dctl)
{
	int res = SM_SUCCESS;
	u8 accel_config2, accel_intel_ctrl, wom, odr;

	INV_DBG_FUNC_NAME;
	i2c_read(dctl, MPU5400_ACCEL_CONFIG2, 1, &accel_config2,
		res, exit_i2c_error);
	i2c_read(dctl, MPU5400_ACCEL_INTEL_CTRL, 1, &accel_intel_ctrl,
		res, exit_i2c_error);

	/*set lpa mode*/
	if(dctl->is_lpa_enabled) {
		accel_config2 |= BIT_ACCEL_FCHOICE_B;
		i2c_write_single(dctl, MPU5400_ACCEL_CONFIG2, accel_config2,
				 res, exit_i2c_error);

		wom = dctl->reg.wom_thres;
		i2c_write_single(dctl, MPU5400_WOM_THRESH, wom, res, exit_i2c_error);

		odr = LPA_CLK_3P91HZ;
		i2c_write_single(dctl, MPU5400_LP_ACCEL_ODR, odr,
				 res, exit_i2c_error);
	...
	if(dctl->is_mi_enabled) {
		...
		i2c_write_single(dctl, MPU5400_ACCEL_INTEL_CTRL, accel_intel_ctrl,
			res, exit_i2c_error);
	} else {
		accel_intel_ctrl &= ~BIT_ACCEL_INTEL_EN;
		accel_intel_ctrl &= ~BIT_ACCEL_INTEL_MODE;
...
static int mpu5400_enable(struct mpu5400_ctrl_t *dctl,
				 u32 func_mask, bool enable)
{
	...
	i2c_write_single(dctl, MPU5400_USER_CTRL, 0, res, exit_i2c_error); //rocky 
	msleep(5);  //rocky 
	...
	/* configurate gyro/accel                                              */
	/*-------------------------------------------------*/
	i2c_read(dctl, MPU5400_PWR_MGMT_1, 2, pwr_mgmt, res, exit_i2c_error);
	...
	if (!mask) {
		...
		*/
		...
		i2c_write_single(dctl, MPU5400_PWR_MGMT_2, pwr_mgmt[1], res,
			exit_i2c_error);
		...
		/*set default smplrt_div*/
		i2c_write_single(dctl, MPU5400_SMPLRT_DIV, 0xff, res, exit_i2c_error);

		/*reset fifo */
		user_ctrl = BIT_FIFO_RST;
		if (dctl->sm_ctrl.is_dmp_on)
			user_ctrl |= BIT_DMP_RST;
		i2c_write_single(dctl, MPU5400_USER_CTRL, user_ctrl, res, exit_i2c_error);

		if(dctl->reg.prog_start_addr != dctl->sm_ctrl.start_addr) {
			u8 regs[2];

			dctl->reg.prog_start_addr = dctl->sm_ctrl.start_addr;
			regs[0] = dctl->reg.prog_start_addr >> 8;
			regs[1] = dctl->reg.prog_start_addr & 0xFF;
			i2c_write_single(dctl, MPU5400_PRGM_STRT_ADDRH, regs[0], res,
					 exit_i2c_error);
...
#if 1 //rocky 
		user_ctrl=0;
		i2c_read_reg(dctl,MPU5400_USER_CTRL,1,&user_ctrl);
		INVSENS_LOGD("%s : user_ctrl2=%x\n", __func__, user_ctrl);
#endif
		...
		if(fifo_en)
			i2c_write_single(dctl, MPU5400_FIFO_EN, fifo_en, res, exit_i2c_error);

		/*set motion interrupt*/
		mpu5400_set_motion_int(dctl);
...
static int mpu5400_postenable(struct mpu5400_ctrl_t *dctl,
				 u32 func_mask, bool enable)
{
	mpu5400_set_delay(dctl, 0, 0, true);
	return SM_SUCCESS;
}



static int mpu5400_apply_orientation(struct mpu5400_ctrl_t *dctl,
...
static int mpu5400_check_dmp_int(struct mpu5400_ctrl_t *dctl,
				 u8 dmp_int,
				 struct invsens_data_list_t *buffer)
{
	INVSENS_LOGD("[INV] %s dmp_int = %d\n", __func__, dmp_int);

	if (dmp_int & 0x02) {
		dctl->is_gyro_requested = true;
		mpu5400_enable(dctl, dctl->enabled_mask, true);
		mpu5400_set_delay(dctl, 0, 0, true);
		INVSENS_LOGD("GYRO ON\n");

	}
	else if (dmp_int & 0x04)
	{
		dctl->is_gyro_requested = false;
		mpu5400_enable(dctl, dctl->enabled_mask, false);
		mpu5400_set_delay(dctl, 0, 0, true);
		INVSENS_LOGD("GYRO OFF\n");
	}

	return SM_SUCCESS;
}

static int mpu5400_read(struct mpu5400_ctrl_t *dctl,
			struct invsens_data_list_t *buffer)
{
	int res = 0, i = 0;
	u8 regs[2];
...
#define MPU_INT (1)
	...
	res = i2c_read_reg(dctl, MPU5400_DMP_INT_STATUS, 2, int_stat);
	if (res)
		goto exit_i2c_error;

	buffer->int_status = int_stat[MPU_INT];
	buffer->dmp_int_status = int_stat[DMP_INT];

	res = i2c_read_reg(dctl, MPU5400_FIFO_COUNTH, 2, regs);
	if (res)
		goto exit_i2c_error;
	...
	if (dctl->sm_ctrl.is_dmp_on) {
		/*
			...
			fifo memory copied on copy_buffer, transfer to fw driver of dmp.
			because, mpu can't know the fifo format.
		*/
		if (count > 0 && buffer->copy_buffer) {
			i2c_read(dctl, MPU5400_FIFO_R_W, count,
		...
		*/
		if (int_stat[DMP_INT]) {
			...
			mpu5400_check_dmp_int(dctl, dmp_int, buffer);
		}
	} else {
		if (count > 0) {
			...
			do {
				i2c_read(dctl, MPU5400_FIFO_R_W, dctl->packet_size, buf,
					res, exit_i2c_error);
				i = 0;
				...
				if (dctl->is_accel_on_fifo) {
					...
					mpu5400_apply_orientation(dctl,
						&buf[i], &p->data[0]);
					i += SENSOR_PACKET;
				}
...
static int mpu5400_read_accel_raw(struct mpu5400_ctrl_t *dctl,
	short raw[AXIS_NUM])
{
	int res = SM_SUCCESS;
	u8 d[SENSOR_PACKET];	

	i2c_read(dctl, MPU5400_ACCEL_XOUT_H, SENSOR_PACKET, d, res,
		exit_i2c_error);

	mpu5400_apply_orientation(dctl, d, (u8 *)raw);
...
static int mpu5400_read_gyro_raw(struct mpu5400_ctrl_t *dctl,
	short raw[AXIS_NUM])
{
	int res = SM_SUCCESS;
	u8 d[SENSOR_PACKET];

	i2c_read(dctl, MPU5400_GYRO_XOUT_H, SENSOR_PACKET, d, res,
		exit_i2c_error);
...
static int mpu5400_get_temperature(struct mpu5400_ctrl_t *dctl, int *temp)
{
	int res = SM_SUCCESS;
	u8 regs[2];
	int temperature;
	...
	i2c_read(dctl, MPU5400_TEMP_OUT_H, 2,
...
static int mpu5400_do_accel_calib(struct mpu5400_ctrl_t *dctl, short *bias)
{
	int res = SM_SUCCESS;
	u8 regs[2], tmp;
	...
	for (i = 0; i <packet_count; i++) {
		...
		mpu5400_apply_orientation(dctl, sensor_raw, (u8 *)sensors);
...
static int mpu5400_if_init(struct invsens_driver_t *drv)
{
	int res = 0;
	struct mpu5400_ctrl_t *dctl;

	INV_DBG_FUNC_NAME;

	dctl = INVSENS_DRV_USER_DATA(drv);

	if ((res = mpu5400_setup_hw(dctl)) != SM_SUCCESS)
		goto exit_hw_setup;
...
static int mpu5400_if_enable(struct invsens_driver_t *drv,
				  u32 func_mask, bool enable,
				  struct invsens_sm_ctrl_t *sm_ctrl)
{
	int res = 0;
	...
	res = mpu5400_enable(dctl, func_mask, enable);
...
static int mpu5400_if_postenable(struct invsens_driver_t *drv,
				  u32 func_mask, bool enable,
				  struct invsens_sm_ctrl_t *sm_ctrl)
{
	struct mpu5400_ctrl_t *dctl;
	...
	return mpu5400_postenable(dctl, func_mask, enable);
}

static int mpu5400_if_set_delay(struct invsens_driver_t *drv,
				     u32 func_mask, long delay)
{
	...
	res = mpu5400_set_delay(dctl, func_mask, delay, false);
	return res;
}

static int mpu5400_if_read(struct invsens_driver_t *drv,
				struct invsens_data_list_t *buffer)
{
	int res = 0;
	struct mpu5400_ctrl_t *dctl = NULL;


	INV_DBG_FUNC_NAME;

	dctl = INVSENS_DRV_USER_DATA(drv);
	res = mpu5400_read(dctl, buffer);
	return res;
}


static int mpu5400_if_terminate(struct invsens_driver_t *drv)
{
	int res = 0;
	struct mpu5400_ctrl_t *dctl;
...
static int mpu5400_if_ioctl(struct invsens_driver_t *drv, u32 cmd,
				 long lparam, void *vparam)
{
	...
	case INV_IOCTL_GYRO_HW_SELFTEST:
		{
			...
			res = mpu5400_hw_selftest(dctl, accel_st_ratio, gyro_st_ratio);
	...
	case INV_IOCTL_GET_ACCEL_RAW:
		{
			...
			mpu5400_read_accel_raw(dctl, accel_raw);
			if(ioctl) {
	...
	case INV_IOCTL_GET_GYRO_RAW:
		{
			...
			mpu5400_read_gyro_raw(dctl, gyro_raw);
			if(ioctl) {
	...
	case INV_IOCTL_S_GYRO_SELFTEST:
		{
			...
			*/
			...
			if(scaled_bias[0] > 30*DEF_ST_PRECISION || 
				...
				st_result = 1; //success
			hwst_result = mpu5400_hw_selftest(dctl, accel_st_ratio, gyro_st_ratio);
			if(ioctl) {
				ioctl->s_gyro_selftest.result = (!(hwst_result & 0x1) | !st_result);
				ioctl->s_gyro_selftest.gyro_ratio[0] = gyro_st_ratio[0];
				ioctl->s_gyro_selftest.gyro_ratio[1] = gyro_st_ratio[1];
				ioctl->s_gyro_selftest.gyro_ratio[2] = gyro_st_ratio[2];
				ioctl->s_gyro_selftest.gyro_bias[0] = scaled_bias[0];
				ioctl->s_gyro_selftest.gyro_bias[1] = scaled_bias[1];
				ioctl->s_gyro_selftest.gyro_bias[2] = scaled_bias[2];
				ioctl->s_gyro_selftest.gyro_rms[0] = scaled_rms[0];
				ioctl->s_gyro_selftest.gyro_rms[1] = scaled_rms[1];
				ioctl->s_gyro_selftest.gyro_rms[2] = scaled_rms[2];
				ioctl->s_gyro_selftest.packet_cnt = packet_count;
			}
		}
		return SM_IOCTL_HANDLED;
	case INV_IOCTL_S_ACCEL_SELFTEST:
		{
			...
			if(ioctl) {
				ioctl->s_accel_selftest.result = !((res >> 1) & 0x1);
				ioctl->s_accel_selftest.accel_ratio[0] = accel_st_ratio[0];
				ioctl->s_accel_selftest.accel_ratio[1] = accel_st_ratio[1];
				ioctl->s_accel_selftest.accel_ratio[2] = accel_st_ratio[2];
			}
		}
		return SM_IOCTL_HANDLED;
	...
	case INV_IOCTL_DO_ACCEL_CAL:
		{
			mpu5400_do_accel_calib(dctl, ioctl->accel_bias.bias);
		}
		return SM_IOCTL_HANDLED;
	...
	case INV_IOCTL_GET_TEMP :
		{
			mpu5400_get_temperature(dctl, &ioctl->temperature.temp);
		}
		return SM_IOCTL_HANDLED;
	}
...
static struct invsens_driver_t mpu5400_handler = {
	.driver_id = INV_DRIVER_MPU65XX,
	.driver_layer = INV_DRIVER_LAYER_NATIVE,
	.func_mask = (1 << INV_FUNC_ACCEL) |(1 << INV_FUNC_GYRO)
		|(1 << INV_FUNC_MOTION_INTERRUPT) |(1 << INV_FUNC_FT_SENSOR_ON),

	.init = mpu5400_if_init,
	.enable = mpu5400_if_enable,
	.postenable = mpu5400_if_postenable,
	.set_delay = mpu5400_if_set_delay,
	.terminate = mpu5400_if_terminate,
	.read = mpu5400_if_read,

	.ioctl = mpu5400_if_ioctl,
	.user_data = NULL,
};

int invsens_load_mpu5400(struct invsens_driver_t **driver,
	struct invsens_board_cfg_t *board_cfg)
{
	...
	dctl = kzalloc(sizeof(struct mpu5400_ctrl_t), GFP_KERNEL);
	if (!dctl) {
		res = -ENOMEM;
		goto error_case;
	}
	...
	mpu5400_handler.user_data = dctl;

	*driver = &mpu5400_handler;
