...
int de_ccsc_set_reg_base(unsigned int sel, unsigned int chno, unsigned int base)
{
...
int de_icsc_set_reg_base(unsigned int sel, unsigned int chno, unsigned int base)
{
	__inf("sel=%d, chno=%d, base=0x%x\n", sel, chno, base);
...
int de_ccsc_apply(unsigned int sel, unsigned int ch_id, struct disp_csc_config *config)
{
	...
	{
		if(config[ch_id].in_fmt == DE_RGB && config[ch_id].out_fmt == DE_RGB)
		{
		...
		else
		{
			...
			in_fmt = config[ch_id].in_fmt;
			in_mode = config[ch_id].in_mode;
			out_fmt = config[ch_id].out_fmt;
			out_mode = config[ch_id].out_mode;
		}
	...
	de_csc_coeff_calc(in_fmt, in_mode, out_fmt, out_mode,
						config[ch_id].brightness, config[ch_id].contrast, config[ch_id].saturation, config[ch_id].hue,
						config[ch_id].out_color_range, csc_coeff);
