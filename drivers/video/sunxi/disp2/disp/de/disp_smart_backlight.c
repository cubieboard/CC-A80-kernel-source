...
s32 disp_smbl_sync(struct disp_smbl* smbl)
{
	...
	if(mgr && mgr->device) {
		if(DISP_OUTPUT_TYPE_LCD == mgr->device->type) {
			struct disp_lcd *lcd = container_of(mgr->device, struct disp_lcd, dispdev);
			if(lcd && lcd->set_bright_dimming)
				lcd->set_bright_dimming(lcd, smblp->info.backlight_dimming);
		}
	}
...
s32 disp_smbl_get_window(struct disp_smbl* smbl, disp_rect *window)
{
	struct disp_smbl_private_data *smblp = disp_smbl_get_priv(smbl);
...
s32 disp_smbl_unset_manager(struct disp_smbl* smbl)
{
	struct disp_smbl_private_data *smblp = disp_smbl_get_priv(smbl);
