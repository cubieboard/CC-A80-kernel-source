...
//**********************************************************************************
// function       : WB_EBIOS_Set_Para(__u32 sel,  __disp_rectsz_t insize, __disp_rect_t cropwin, __disp_rect_t outwin, __disp_fb_t outfb)
...
//***********************************************************************************
__s32 WB_EBIOS_Set_Para(__u32 sel,  __disp_rectsz_t insize, __disp_rect_t cropwin, __disp_rect_t outwin, __disp_fb_t outfb)
{
	__u32 sclfac;
	...
	if(outfb.format == DISP_FORMAT_ARGB8888 || outfb.format == DISP_FORMAT_ARGB8888)
	{
		wb_dev[sel]->lstrd0.bits.lstrd = outfb.size.width * 4;
		wb_dev[sel]->format.bits.format = 0;
		wb_dev[sel]->format.bits.ps = (outfb.seq == DISP_SEQ_ARGB)?1:0;
	...
	else if(outfb.format == DISP_FORMAT_YUV420)
	{
		...
		wb_dev[sel]->format.bits.ps = (outfb.seq == DISP_SEQ_UVUV)?0:1;
