...
s32 disp_lcd_vga_is_used(struct disp_lcd* lcd)
{
   struct disp_lcd_private_data *lcdp = disp_lcd_get_priv(lcd);
...
//lcd_used
    ...
    if(lcd_cfg->lcd_used == 0) //no need to get lcd config if lcd_used eq 0
        return ;
    // vga_used
    ret = OSAL_Script_FetchParser_Data(primary_key, "vga_used", &value, 1);
	if(ret == 0)
    {
        lcd_cfg->vga_used = value;
    }
//lcd_bl_en
...
s32 disp_lcd_set_vga_mode(struct disp_lcd *lcd, disp_tv_mode vga_mode)
{
	disp_panel_para* info;
	s32 ret = -1;
	struct disp_lcd_private_data *lcdp = disp_lcd_get_priv(lcd);
	...
	if(!lcd->is_vga_used(lcd)){
		DE_WRN("VGA is not used!\n");
		return DIS_FAIL;
	}
	info = &(lcdp->panel_info);
	switch (vga_mode){
	case DISP_TV_MOD_VGA_H1024_V768:{
		printk(KERN_ERR "##vga 1024*768\n");
		info->lcd_if = 0;
		info->lcd_x = 1024;
		info->lcd_y = 768;
		info->lcd_dclk_freq = 65;
		info->lcd_ht = 1344;
		info->lcd_hbp = 296;
		info->lcd_hspw = 136;
		info->lcd_vt  = 806;
		info->lcd_vbp = 35;
		info->lcd_vspw = 6;
		lcdp->mode = DISP_TV_MOD_VGA_H1024_V768;
		ret = 0;
		}break;
	case DISP_TV_MOD_VGA_H800_V600:{
		printk(KERN_ERR "##vga H800_V600\n");
		info->lcd_if = 0;
		info->lcd_x = 800;
		info->lcd_y = 600;
		info->lcd_dclk_freq = 40;
		info->lcd_ht = 1056;
		info->lcd_hbp = 216;
		info->lcd_hspw = 128;
		info->lcd_vt  = 628;
		info->lcd_vbp = 27;
		info->lcd_vspw = 4;
		lcdp->mode = DISP_TV_MOD_VGA_H800_V600;
		ret = 0;
	    }break;
	case DISP_TV_MOD_VGA_H1400_V1050:{
		printk(KERN_ERR "##vga H1400_V1050\n");
		info->lcd_if = 0;
		info->lcd_x = 1400;
		info->lcd_y = 1050;
		info->lcd_dclk_freq = 122;
		info->lcd_ht = 1864;
		info->lcd_hbp = 376;
		info->lcd_hspw = 144;
		info->lcd_vt  = 1089;
		info->lcd_vbp = 32;
		info->lcd_vspw = 4;
		lcdp->mode = DISP_TV_MOD_VGA_H1400_V1050;
		ret = 0;
		}break;
	default :{
        printk(KERN_ERR "##vga default\n");
		info->lcd_if = 0;
		info->lcd_x = 1024;
		info->lcd_y = 768;
		info->lcd_dclk_freq = 65;
...
s32 disp_lcd_get_vga_mode(struct disp_lcd *lcd)
{
    s32 vga_mode = -1;
	struct disp_lcd_private_data *lcdp = disp_lcd_get_priv(lcd);
...
s32 disp_init_lcd(__disp_bsp_init_para * para)
{
	...
	for(screen_id=0; screen_id<num_screens; screen_id++) {
		...
		lcd->is_vga_used = disp_lcd_vga_is_used;
        ...
        lcd->set_vga_mode  = disp_lcd_set_vga_mode;
		lcd->get_vga_mode  = disp_lcd_get_vga_mode;
