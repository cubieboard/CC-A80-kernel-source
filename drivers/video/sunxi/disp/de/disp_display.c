...
s32 bsp_disp_lcd_get_vga_mode(u32 screen_id)
{
       struct disp_lcd *lcd = disp_get_lcd(screen_id);
       if(lcd && lcd->get_vga_mode) {
          return lcd->get_vga_mode(lcd);
       }
       return DIS_FAIL;
}

s32 bsp_disp_lcd_set_vga_mode(u32 screen_id, disp_tv_mode vga_mode)
{
     struct disp_lcd *lcd = disp_get_lcd(screen_id);
     if(lcd && lcd->set_vga_mode) {
        return lcd->set_vga_mode(lcd, vga_mode);
     }
     return DIS_FAIL;
}
