...
static void start_work(struct work_struct *work)
{
	...
	for(screen_id = 0; screen_id<num_screens; screen_id++) {
		...
		if(((g_fbi.disp_init.disp_mode	== DISP_INIT_MODE_SCREEN0) && (screen_id == 0))
		   ...
		   if(g_fbi.disp_init.output_type[screen_id] == DISP_OUTPUT_TYPE_HDMI) {
#if defined(CONFIG_ARCH_SUN9IW1P1)
		...
		}else if (((g_fbi.disp_init.disp_mode	== DISP_INIT_MODE_SCREEN0) && (screen_id == 0))
			|| ((g_fbi.disp_init.disp_mode	== DISP_INIT_MODE_SCREEN1) && (screen_id == 1))) {
...
long disp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	#if defined(CONFIG_ARCH_SUN9IW1P1)
	    ret = bsp_disp_lcd_set_vga_mode(ubuffer[0], ubuffer[1]);
	#endif
        break;
