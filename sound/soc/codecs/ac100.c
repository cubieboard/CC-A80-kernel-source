...
static bool codec_headsetmic_voicerecord = true;
...
{
	struct headset_data *hs_data = container_of(work, struct headset_data, state_work);
	switch_status_update(hs_data);
...
{
	struct headset_data	*hs_data =(struct headset_data *)data;
	if (gpio_get_value(hp_det.gpio.gpio) == 0) {
		hs_data->state = 0;
	} else {
		hs_data->state = 1;
	}
	//switch_status_update(hs_data);
	if(0 == queue_work(switch_queue, &hs_data->state_work)){
		//irq_flag--;
		printk("[switch]add work struct failed!\n");
	}
	mod_timer(&hs_data->timer, jiffies +  HZ/2);
}
#endif
...
static int sndvir_audio_soc_probe(struct snd_soc_codec *codec)
{
	...
	switch_queue = create_singlethread_workqueue("switch_irq");
	...
	type = script_get_item("audio0", "audio_hp_det", &hp_det);
	if (SCIRPT_ITEM_VALUE_TYPE_PIO != type) {
		printk("script_get_item return type err\n");
		return -EFAULT;
	}
	req_status = gpio_request(hp_det.gpio.gpio, NULL);
	if (0 != req_status) {
	...
	*/
	...
	type = script_get_item("audio0", "speaker_val", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
		printk("[CODEC] speaker_val type err!\n");
	}
	speaker_val = val.val;
